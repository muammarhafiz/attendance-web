...
// IMPORTANT: service role key on server side only
...
async function fetchPeriod(year:number, month:number) {
  ...
  return data as { id:string; status:'OPEN'|'LOCKED' };
}
...
function summaryPdf(year:number, month:number, rows:Row[]) {
  return makePdfBuffer((doc) => {
    ...
    doc.moveDown();
    doc.fontSize(10).text(['Employee','Gross','Base','EPF(E)','SOCSO(E)','EIS(E)','Manual','Net','EPF(Er)','SOCSO(Er)','EIS(Er)'].join(' | '));
    doc.moveDown(0.25);
    rows.forEach(r => {
      const line = [
...
function payslipPdf(year:number, month:number, staff:any, summaryRow:Row, lines:any[]) {
  return makePdfBuffer((doc) => {
    ...
    doc.font('Helvetica-Bold').text('Earnings'); doc.font('Helvetica');
    lines.filter(l => l.kind === 'EARN').forEach(l => {
      doc.text(`${l.label || l.code} — RM ${Number(l.amount).toFixed(2)}`);
    });

    doc.moveDown();
    doc.font('Helvetica-Bold').text('Deductions'); doc.font('Helvetica');
...
async function uploadToStorage(path:string, bytes:Buffer, contentType='application/pdf') {
  ...
  if (error) throw new Error(error.message);
  const { data } = supabaseAdmin.storage.from('payroll').getPublicUrl(path);
...
export async function POST(req: Request) {
  try {
    ...
    if (!rows.length) {
      return NextResponse.json({ error:'No payroll rows for this period' }, { status:400 });
    }
    ...
    const summaryUrl = await uploadToStorage(`${basePath}/Payroll_Summary_${basePath}.pdf`, summaryBuf);

    // Payslips (bounded concurrency)
    ...
    await Promise.all(
      Array.from({ length: concurrency }).map(async () => {
        for (;;) {
          ...
          const ls = items.filter(i => i.staff_email === email);
          const s  = staffMap.get(email);
          const buf = await payslipPdf(year, month, s, r, ls);
          const safeName = (s?.full_name || s?.name || email).replace(/[^\w\-]+/g, '_') || email;
          const url = await uploadToStorage(`${basePath}/payslips/${safeName}_${basePath}.pdf`, buf);
          payslips.push({ email, url });
        }
      })
    );

    // LOCK the period (schema-qualified RPC)
    const { error: lockErr } = await supabaseAdmin
      .schema('pay_v2')
      .rpc('finalize_period', { p_year: year, p_month: month });
