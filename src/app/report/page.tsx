...
export default function ReportPage() {
  ...
  const latePill = (mins: number) =>
    ({ display:'inline-block', padding:'2px 8px', borderRadius:9999, background: mins>0 ? '#fee2e2' : '#e5f6ff', border:'1px solid #e5e7eb' } as const);
        ...
        <table style={{width:'100%', borderCollapse:'separate', borderSpacing:0}}>
          ...
          <tbody>
            ...
            {filtered.map((r, i) => {
              ...
              const showLate = r.action === 'Check-in';
              ...
              return (
                <tr key={i} style={{...row, background: mins > 0 && showLate ? '#fff1f2' : undefined}}>
                  <td style={td}>{kl}</td>
                  ...
                  <td style={td}>
                    {showLate
                      ? <span style={latePill(mins)}>{mins} min</span>
                      : <span style={latePill(0)}>0 min</span>}
                  </td>
