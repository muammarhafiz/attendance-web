...
export default function AdminPayrollDashboard() {
  ...
  const loadData = async () => {
    ...
    // must be authenticated client-side
    const { data: u } = await supabase.auth.getUser()
    if (!u.user) {
      setMsg('You are not signed in on the client. Tap “Sign out”, then sign in again.')
      setStatus('N/A')
      setRows([]); setCompanyTotals(null)
      setLoading(false)
      return
    }
  ...
  // ---- Admin actions (RPCs are also schema-scoped) ----
      ...
      <header className="mb-6 flex items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-xs text-gray-500">
            Client auth: {userEmail ?? 'not signed in'}
          </p>
        </div>
