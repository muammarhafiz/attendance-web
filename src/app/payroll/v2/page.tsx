...
export default function PayrollV2Page() {
  ...
  const refresh = useCallback(async () => {
    setLoading(true);
    try {
      // 1) Period status (use a light view if you have it; fallback to base table)
      {
  ...
  /* ============================================================
     DETAILS MODAL
     - Deduction list now includes UNPAID as a first-class row.
     - Edit Unpaid by setting FINAL total via set_unpaid_total().
  ============================================================ */
  ...
  const loadManualAndUnpaid = useCallback(
    async (emailAddr: string) => {
      ...
      // Fetch UNPAID plumbing items for final total (EARN/UNPAID_ADJ & DEDUCT/UNPAID_EXTRA)
      if (period?.id) {
        ...
        if (!plErr && Array.isArray(plumb)) {
          let adj = 0, extra = 0;
          for (const r of plumb as {kind:string; code:string|null; amount:number|string}[]) {
  ...
  // ----- Unpaid: edit final total (in DEDUCTION list) -----
          ...
          <div className="mx-auto w-full max-w-3xl rounded-lg bg-white shadow-xl">
            ...
            <div className="flex flex-wrap gap-2 px-4 pt-3">
              <span className="rounded-full bg-gray-100 px-3 py-1 text-xs text-gray-700">
                Auto UNPAID: RM {fmt(sel.unpaid_auto, false)} · {absentMap[sel.staff_email] ?? 0}d
              </span>
              <span className="rounded-full bg-gray-100 px-3 py-1 text-xs text-gray-700">
