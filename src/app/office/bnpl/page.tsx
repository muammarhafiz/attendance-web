'use client';
// Office → BNPL: the Buy-Now-Pay-Later tracker (ATOME today; any future BNPL slots in the same way).
// Two sides: (1) the SALES feed — every BNPL receipt from the cash book, with the fee taken out so you
// see what you actually earn; (2) the PAYOUTS — ATOME's batched bank deposits, imported from their
// settlement report, each ticked once the clerk sees it land in the bank. Plus the total fees for P&L.
...
export default function BnplPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const { data } = await supabase.rpc('bnpl_overview', { p_from: from, p_to: to });
  ...
  const onFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      const r = (j.result ?? {}) as { rows?: number; payouts?: number };
      setMsg({ ok: true, text: `Imported ${r.rows ?? 0} transaction${r.rows === 1 ? '' : 's'} across ${r.payouts ?? 0} payout${r.payouts === 1 ? '' : 's'}.` });
      await load();
    } catch (e) {
  ...
  const confirmPayout = useCallback(async (p: Payout, confirmed: boolean) => {
    ...
    const { error } = await supabase.rpc('bnpl_confirm_payout', { p_provider: 'atome', p_payout_id: p.payout_id, p_confirmed: confirmed, p_note: p.note ?? null });
    if (error) load();
  }, [load]);
  ...
  return (
    <OfficeShell title="BNPL payments" back onRefresh={load}>
      <p className="mb-4 text-sm text-ink-2">
        When a customer pays with ATOME, you get the sale now but ATOME keeps a fee and pays you the rest a few days later,
        bundled into one bank deposit. Import the settlement file ATOME emails you, then tick each payout once you see it in the bank.
      </p>

      {/* Import the settlement report */}
      <div className="mb-4 rounded-card bg-card shadow-card p-4">
        <div className="mb-1 text-sm font-semibold text-ink">Import ATOME payout report</div>
        <p className="mb-3 text-[13px] text-ink-2">
          Open the email ATOME sent, download the <span className="font-medium">settlement / payout</span> spreadsheet, then drop it here.
        </p>
      ...
      {loading || !d ? (
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-card bg-card shadow-card p-4">
            ...
            <p className="mb-3 text-[12px] text-ink-3">Each row is one deposit ATOME made. Tick it once you see the matching amount in the bank statement on that date.</p>
            {d.payouts.length === 0 ? (
          ...
          {d.providers[0] && (
            <p className="px-1 text-[11px] text-ink-3">
              Fee model — {d.providers.map((p) => `${p.display_name}: ${p.mdr_pct}% + RM${p.flat_fee} (+${p.sst_pct}% SST)`).join(' · ')}.
            </p>
          )}
        </div>
      )}
