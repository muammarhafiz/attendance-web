...
export default function OffdayPage() {
  ...
  // Admin gate — with authChecked + authError so it never hangs forever
  useEffect(() => {
        let mounted = true;
        const checkAuth = async () => {
                try {
                          ...
                          if (!mounted) return;
                          if (sRes.error) {
                                      setAuthError(sRes.error.message);
                                      setAuthChecked(true);
                                      return;
                          }
                          ...
                          if (!mounted) return;
                          setAuthError(e?.message ?? 'Failed to verify session. Please refresh.');
                          setAuthChecked(true);
                }
        };
        checkAuth();
  ...
  const saveRange = async () => {
        if (!isAdmin) return;
        setErr('');
        if (!fromDay || !toDay) { alert('Please select From and To dates.'); return; }
        const days = eachDateInclusive(fromDay, toDay);
        if (days.length === 0) { alert('Invalid date range.'); return; }
        if (days.length >= 60) { alert('Date range too large. Please keep it under 60 days.'); return; }
        ...
        try {
                ...
                for (const d of days) {
                          const { error } = await supabase.rpc('set_day_status', { p_email: selected, p_day: d, p_status: status, p_note: note || null });
                          if (error) throw error;
                }
                await loadMonth();
  ...
  const clearRange = async () => {
        ...
        try {
                ...
                for (const d of days) {
                          const { error } = await supabase.rpc('clear_day_status', { p_email: selected, p_day: d });
                          if (error) throw error;
                }
                await loadMonth();
  ...
  const input = 'w-full rounded border px-3 py-2';
    const btn = 'rounded border px-3 py-2 text-sm font-medium hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed';

  // Auth error state — show error with retry instead of hanging forever
  if (authError) {
        return (
                <main className="mx-auto max-w-4xl p-6">
                        <div className="rounded border border-red-200 bg-red-50 p-4 text-sm text-red-700">
                                  ...
                                  <button
                                                ...
                                                onClick={() => { setAuthError(''); setAuthChecked(false); window.location.reload(); }}
                                              >
                                              Retry
                                  </button>button>
                        </div>div>
