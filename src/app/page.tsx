...
export default function Home() {
  ...
  const submit = async (action: 'Check-in' | 'Check-out') => {
    ...
    statusEl.innerHTML =
      ...
      `Distance to workshop: <b>${d}</b> m ${d>RADIUS_M ? '<span style="color:#b91c1c">(outside radius)</span>' : ''}`;

    msgEl.style.color='black'; msgEl.textContent='Submitting…';
    ...
    if (error) { msgEl.style.color='red'; msgEl.textContent = error.message; return; }
  ...
  return (
    <main style={{ padding:16, fontFamily:'system-ui' }}>
      ...
      {/* Map centers on workshop first; after GPS arrives it auto-zooms to YOUR pin */}
      ...
      <div style={{ border:'1px solid #ddd', borderRadius:8, padding:16, margin:'12px 0' }}>
        <button onClick={() => submit('Check-in')}
          style={{ width:'100%', padding:14, border:0, borderRadius:8, background:'#16a34a', color:'#fff', fontSize:16, marginTop:6 }}>
          Check in
        </button>
        <button onClick={() => submit('Check-out')}
          style={{ width:'100%', padding:14, border:0, borderRadius:8, background:'#0ea5e9', color:'#fff', fontSize:16, marginTop:6 }}>
          Check out
        </button>
