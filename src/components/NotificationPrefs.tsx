...
function Switch({ on, onChange, disabled }: { on: boolean; onChange: (v: boolean) => void; disabled?: boolean }) {
  return (
    <button type="button" role="switch" aria-checked={on} disabled={disabled} onClick={() => onChange(!on)}
      className={`relative inline-flex h-6 w-11 shrink-0 items-center rounded-full transition disabled:opacity-50 ${on ? 'bg-good' : 'bg-ink/15'}`}>
...
export default function NotificationPrefs() {
  ...
  return (
    <div className="rounded-xl border border-line bg-card p-4">
      <h2 className="text-sm font-semibold text-ink">Which alerts push to your phone</h2>
      <p className="mt-1 text-xs text-ink-2">Turn off any alert you don&rsquo;t want as a phone notification. (It still appears in the in-app bell.)</p>
      <ul className="mt-3 divide-y divide-line">
        {prefs.map((p) => (
          <li key={p.type} className="flex items-center justify-between gap-3 py-2">
            <span className="text-sm text-ink-2">{TYPE_LABEL[p.type] ?? p.type}</span>
