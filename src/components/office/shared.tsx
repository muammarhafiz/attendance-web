...
export function UnpaidCard({ unpaid }: { unpaid: Home['unpaid'] }) {
  return (
    <Card title="Chase unpaid" icon="📞">
      {unpaid.count > 0 ? (
        <>
          <div className="mb-2 text-sm text-gray-600"><span className="font-semibold text-gray-900">{rm0(unpaid.total)}</span> across {unpaid.count} invoice{unpaid.count === 1 ? '' : 's'}</div>
          ...
          {unpaid.count > unpaid.top.length && <div className="mt-1 text-[11px] text-gray-400">Top {unpaid.top.length} shown · {unpaid.count - unpaid.top.length} more</div>}
        </>
