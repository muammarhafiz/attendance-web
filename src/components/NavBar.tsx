              ...
              <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
                      <div className="flex items-center gap-1 sm:gap-2">
                                ...
                                <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
                                            <Link href="/" prefetch={false} className={linkClass('/')}>Check-in</Link>Link>
                                            <Link href="/today" prefetch={false} className={linkClass('/today')}>Today</Link>Link>
                                            <Link href="/report" prefetch={false} className={linkClass('/report')}>Report</Link>Link>
                                            <Link href="/offday" prefetch={false} className={linkClass('/offday')}>Offday/MC</Link>Link>
                                  {isAdmin && (
                        <>
                                        <Link href="/payroll/records" prefetch={false} className={linkClass('/payroll/records')}>Payroll Records</Link>Link>
                                        <Link href="/payroll/v2" prefetch={false} className={linkClass('/payroll/v2')}>Payroll v2</Link>Link>
                        </>>
                      )}
                                            <Link href="/employees" prefetch={false} className={linkClass('/employees')}>Employees</Link>Link>
                                </div>div>
                      </div>div>
                      <div className="flex items-center gap-2 sm:gap-3">
                        {email ? (
                      <>
                                    <span className="hidden text-sm text-gray-600 sm:inline">{email}</span>span>
                                    <button
