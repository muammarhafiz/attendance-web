...
const modeStyle = (m: string) =>
  m === 'workshop' ? 'bg-accent-weak text-accent' : m === 'home' ? 'bg-good-soft text-good' : 'bg-ink/5 text-ink-3';
  ...
  return (
    <div className="max-w-xl">
      <div className="mb-4 rounded-card bg-card shadow-card p-4">
        <div className="text-sm font-medium text-ink-2">Shifts &amp; weekly schedule per employee</div>
        ...
        <table className="w-full border-collapse text-sm">
          <thead>
            <tr className="bg-ink/5 text-left">
              <th className="px-3 py-2 font-medium text-ink-2">Employee</th>
              <th className="px-3 py-2 font-medium text-ink-2">Start</th>
              <th className="px-3 py-2 font-medium text-ink-2">Weekly schedule (tap a day)</th>
            </tr>
          </thead>
          <tbody>
            {staff.map((s) => {
              ...
              return (
                <tr key={s.email} className="border-t border-line">
                  <td className="whitespace-nowrap px-3 py-2 align-top text-ink">
                    {s.name ?? s.email}
                    {savingEmail === s.email && <span className="ml-1 text-xs text-ink-3">saving…</span>}
                  </td>
            ...
            {staff.length === 0 && (
              <tr><td colSpan={3} className="px-3 py-6 text-center text-ink-2">No staff found.</td></tr>
            )}
          </tbody>
        </table>
      </div>
      <div className="mt-2 flex flex-wrap gap-3 text-xs text-ink-2">
        <span><span className="mr-1 inline-block h-3 w-3 rounded bg-accent-weak align-middle" />Workshop (GPS)</span>
        <span><span className="mr-1 inline-block h-3 w-3 rounded bg-good-soft align-middle" />Home (auto-present)</span>
        <span><span className="mr-1 inline-block h-3 w-3 rounded bg-ink/5 align-middle" />Off</span>
      </div>
      ...
      <div className="mt-4 flex items-center gap-3">
        <button
          ...
          className="rounded-md border border-line px-3 py-1.5 text-sm text-ink-2 hover:bg-ink/5 disabled:opacity-50"
        >
          {recomputing ? 'Recomputing…' : 'Recompute all history'}
        </button>
        <span className="text-xs text-ink-3">Use after changing a shift, to fix past months&apos; late numbers.</span>
      </div>

      {msg && <div className="mt-3 text-sm text-good">{msg}</div>}
    </div>
  );
}
